...
export default function ProductCard({ product, delay = 0 }: { product: Product; delay?: number }) {
  ...
  return (
    <article
      className="clip-card group relative flex flex-col border border-royal/60 bg-gradient-to-b from-navy/70 to-ink transition-all duration-300 hover:-translate-y-1.5 hover:border-neon/60 hover:shadow-[0_18px_50px_rgba(0,168,255,0.18)]"
      style={{ transitionDelay: `${delay}ms` }}
    >
      {/* image area */}
      <Link to={`/product/${product.id}`} className="relative block h-52 overflow-hidden sm:h-56" ariaLabel={`View ${product.name}`}>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_35%,rgba(16,61,128,0.5),transparent_70%)]" />
        <ProductVisual
          product={product}
          className="absolute inset-0"
          imgClass="group-hover:scale-[1.07]"
        />
        {/* scan sweep on hover */}
        ...
        <span className="absolute left-3 top-3 flex flex-col items-start gap-1.5">
          {product.badge && (
            <span className="clip-tag bg-gold px-2.5 py-1 font-display text-[13px] tracking-[0.14em] text-ink">{product.badge}</span>
          )}
          {discount > 0 && (
            <span className="clip-tag border border-neon/60 bg-ink/80 px-2 py-0.5 font-display text-[12px] tracking-[0.14em] text-neon">-{discount}%</span>
          )}
        </span>
        ...
        <button
          ...
          className={`clip-tag absolute right-3 top-3 flex h-9 w-9 items-center justify-center border transition-all duration-200 ${
            wished
              ...
              : "border-royal/70 bg-ink/70 text-frost/60 hover:border-neon hover:text-neon"
          }`}
        ...
        {/* quick view */}
        <span className="clip-btn pointer-events-none absolute bottom-3 left-1/2 flex -translate-x-1/2 translate-y-3 items-center gap-2 bg-navy/95 px-4 py-2 font-display text-sm tracking-[0.22em] text-neon opacity-0 shadow-neon transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <IconEye className="h-4 w-4" /> QUICK VIEW
        </span>
      </Link>

      {/* info */}
      <div className="flex flex-1 flex-col gap-2 border-t border-royal/40 p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-frost/40">({product.reviews})</span>
        </div>
        ...
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          <div className="leading-none">
            ...
            <span className="font-display text-2xl tracking-wide text-gold">{formatNaira(product.price)}</span>
          </div>
        </div>

        <AddToCartButton product={product} className="mt-2 w-full py-2.5 text-base" />
      </div>
    </article>
  );
}
